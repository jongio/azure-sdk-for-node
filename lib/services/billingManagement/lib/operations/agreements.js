...
function _listByBillingAccountName(billingAccountName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountName}/agreements';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['AgreementListResult']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _get(billingAccountName, agreementName, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/billingAccounts/{billingAccountName}/agreements/{agreementName}';
  requestUrl = requestUrl.replace('{billingAccountName}', encodeURIComponent(billingAccountName));
  ...
  return client.pipeline(httpRequest, (err, response, responseBody) => {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['Agreement']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        let deserializationError = new Error(`Error ${error} occurred in deserializing the responseBody - ${responseBody}`);
...
class Agreements {
   ...
   * @reject {Error} - The error object.
   */
  getWithHttpOperationResponse(billingAccountName, agreementName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._get(billingAccountName, agreementName, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  get(billingAccountName, agreementName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._get(billingAccountName, agreementName, options, optionalCallback);
    }
  }
