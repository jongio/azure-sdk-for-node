 ...
 */
function _validate(address, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'providers/Microsoft.Billing/validateAddress';
  let queryParameters = [];
  ...
  try {
    if (address !== null && address !== undefined) {
      let requestModelMapper = new client.models['Address']().mapper();
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['ValidateAddressResponse']().mapper();
...
class Addresses {
   ...
   */
  validateWithHttpOperationResponse(address, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._validate(address, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  validate(address, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._validate(address, options, optionalCallback);
    }
  }
